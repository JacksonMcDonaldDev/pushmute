#!/usr/bin/env bash
#
# Rebuild and relaunch the pushmute dev daemon, guaranteeing no stale instance
# survives to trip the "another pushmute daemon is already running" startup guard
...
sock="${XDG_RUNTIME_DIR:-/tmp}/pushmute.sock"
...
systemctl --user stop pushmute 2>/dev/null || true

# 3. Stop any running dev daemon. SIGTERM (not -9) lets it restore the default
#    source, kill its pw-loopback child, and remove the socket. `pushmute run`
#    re-execs as target/{debug,release}/pushmute, so match the binary, not "cargo".
#    The [s] keeps the pattern from matching this script's own command line.
pkill -TERM -f '[p]ushmute run' 2>/dev/null || true
...
for _ in $(seq 1 50); do
    pgrep -f '[p]ushmute run' >/dev/null || break
    sleep 0.1
done
rm -f "$sock"
