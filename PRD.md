# Selective Mic Router (PushMute) - v1 PRD
...
need for a third-party virtual-cable driver. PushMute creates one virtual source and controls
...
- All other apps use the **default source**, which PushMute sets to its virtual mic ("PushMute").
- The user binds the same physical key as both (a) their comms app's PTT and (b) PushMute's
...
1. PushMute creates its virtual source ("PushMute") in the PipeWire graph. No driver install,
   no download — PipeWire supports virtual devices natively.
2. PushMute prompts (or, on a configured non-interactive start, silently applies) setting the
   **default source** to "PushMute". A `--no-prompt` / config flag suppresses the prompt
...
3. The user configures (via the config file or a small settings UI):
   - The **physical microphone** to route from (selected from detected capture devices,
     excluding PushMute's own virtual mic to prevent feedback loops).
   - The **PTT key** (any key or mouse button, identified by evdev keycode).
   - The **comms app** is pinned to the physical device by the user inside that app — PushMute
     does not need to manage the comms app.
4. PushMute backgrounds itself (systemd user service or `--daemon`) and begins routing.

## Functional Requirements

### FR-1: Virtual Microphone Provisioning

- On startup, create a virtual source named "PushMute" in the PipeWire graph (e.g. a
...
- On startup (after the virtual source exists), set "PushMute" as the PipeWire **default
...
- Best-effort restoration only — if the daemon is killed, "PushMute" remains default until
  the user resets it or restarts PushMute. A `--restore` subcommand should reset the default
...
- **Exclude PushMute's own virtual source** from the list to avoid feedback loops.
...
- Link the selected physical mic into the "PushMute" virtual source within the PipeWire
...
- Because routing is graph linking rather than a manual capture→render copy loop, the
  data path stays inside PipeWire; PushMute's job is to create nodes/links and toggle mute,
...
- **Wayland note:** Wayland deliberately blocks X11-style global keyboard hooks. PushMute
  ...
  - PushMute should let the user pick which input device(s) to listen on, or grab all keyboard/
    mouse devices, and identify the bind by evdev keycode.
- While the key is **held**: mute the loopback link into "PushMute" (set node mute / volume
...
- Optional secondary binding path: a Hyprland `bind`/`bindr` (release) pair that calls an
  PushMute CLI (`pushmute mute --hold` / `pushmute unmute`). This is compositor-specific and a fallback;
...
**Daemon + CLI (primary):**
- PushMute runs as a background daemon (foreground `--daemon` flag and/or a systemd user
  service).
...
- Hyprland has no built-in system tray. Provide status via a **StatusNotifierItem** tray
  ...
  custom module that polls `pushmute status`.
...
- Provide a **systemd user service** (`pushmute.service`) the user can enable with
  `systemctl --user enable --now pushmute`.
- On auto-start, PushMute comes up in the background, provisions the virtual source, applies
...
- **Resource usage:** minimal CPU/memory. It is a background utility, not a DAW; routing
  lives in the PipeWire graph, so PushMute itself should be near-idle.
- **Single instance:** only one PushMute daemon at a time (lock file under
...
| Config persistence | TOML/JSON under `$XDG_CONFIG_HOME/pushmute/` (`~/.config/pushmute/`) | XDG-compliant, human-readable. |
| Autostart | systemd **user** service (`pushmute.service`) | Native Linux autostart. Hyprland `exec-once` documented as alternative. |
...
2. **Default-source restoration robustness:** if the daemon is killed (not gracefully
   exited), "PushMute" stays default. Should the systemd unit add an `ExecStopPost` /
   watchdog that runs `pushmute restore`, and should startup detect "default is a stale PushMute
